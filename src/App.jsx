...
const COMPANIONS = {
  ...
  other: { label: "Other", color: "#7A8699", bg: "rgba(122,134,153,0.15)" },
};

function companionLabel(s) {
  if (s.companion === "other" && s.otherLabel && s.otherLabel.trim()) return s.otherLabel.trim();
  return COMPANIONS[s.companion]?.label ?? s.companion;
}
...
export default function App() {
  ...
  const [manualForm, setManualForm] = useState({ date: todayStr(), startTime: "16:00", endTime: "16:30", companion: "mom", otherLabel: "" });
  ...
  const startSession = () => {
    const now = new Date();
    persistActive({
      id: uid(), date: toLocalDateStr(now), startTime: now.toISOString(), companion: pendingCompanion,
      otherLabel: pendingCompanion === "other" ? pendingOtherLabel.trim() : "",
    });
  };
  ...
  const openManual = () => {
    setEditingId(null);
    setManualForm({ date: todayStr(), startTime: "16:00", endTime: "16:30", companion: (role === "mom" || role === "dad") ? role : "mom", otherLabel: "" });
    setSheet("manual");
  };

  const editSession = (s) => {
    setEditingId(s.id);
    setManualForm({ date: s.date, startTime: toLocalTimeStr(s.startTime), endTime: toLocalTimeStr(s.endTime), companion: s.companion, otherLabel: s.otherLabel || "" });
    setSheet("manual");
  };
  ...
  const submitManual = () => {
    ...
    const record = {
      ...
      otherLabel: manualForm.companion === "other" ? manualForm.otherLabel.trim() : "",
    };
        ...
        <div style={{ background: theme.card, borderRadius: theme.radius, boxShadow: active ? `0 0 0 1.5px ${theme.accent}, ${theme.shadow}` : theme.shadow, padding: "24px", marginBottom: "20px" }}>
          {!active ? (
            <>
              ...
              <div className="mb-5">
                <CompanionToggle value={pendingCompanion} onChange={setPendingCompanion} theme={theme} />
                {pendingCompanion === "other" && (
                  <input
                    type="text" placeholder="e.g. Instructor" value={pendingOtherLabel}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 40 }}>
          ...
          <div className="max-w-md mx-auto" style={{
            ...
            <div className="mb-5">
              ...
              {manualForm.companion === "other" && (
                <input
                  type="text" placeholder="e.g. Instructor" value={manualForm.otherLabel}
                  onChange={(e) => setManualForm((f) => ({ ...f, otherLabel: e.target.value }))}
                  style={{ ...inputStyle, marginTop: "8px" }}
                />
              )}
            </div>
